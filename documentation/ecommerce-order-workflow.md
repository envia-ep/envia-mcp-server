...
1. Use `envia_quote_shipment` to compare rates for the origin/destination
2. Choose a carrier and service from the results
3. Use `envia_create_shipment` with the chosen carrier and the address data from the order
...
The MCP `envia_get_ecommerce_order` tool covers **Step 1** (payload preparation). Steps 2-3 correspond to `envia_create_shipment`. Steps 4-5 (store fulfillment and label download) will be implemented in a future iteration.
