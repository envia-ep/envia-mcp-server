 ...
 * creation using the existing envia_quote_shipment / envia_create_shipment tools.
 */
...
function formatOutput(
    ...
    for (const loc of locations) {
        ...
        if (
            ...
            lines.push('  Use envia_classify_hscode to look up the correct code for each product.');
...
function formatQuotePayload(loc: TransformedLocation): string[] {
    ...
    lines.push('--- Quote Payload (for envia_quote_shipment) ---');
...
function formatGeneratePayload(loc: TransformedLocation): string[] {
    ...
    if (!loc.generatePayload) {
        lines.push('--- Generate Payload (for envia_create_shipment) ---');
        lines.push('  Not available — no carrier pre-selected. Use envia_quote_shipment first to choose a carrier and service.');
        return lines;
    }
...
function formatNextSteps(
    ...
    if (!hasCarrier) {
        lines.push('  • No carrier pre-selected. Use envia_quote_shipment with the origin/destination postal codes and package weight to compare rates.');
        lines.push('  • Once you choose a carrier and service, use envia_create_shipment with the full address details above.');
    } else if (payloadType === 'quote') {
        lines.push('  • A carrier is already pre-selected. You can proceed directly to envia_create_shipment.');
        lines.push('  • Or use envia_quote_shipment to compare other carriers first.');
    } else {
        lines.push('  • Use envia_create_shipment with the carrier, service, and address details shown above to purchase a label.');
        lines.push('  • Or use envia_quote_shipment first to compare other carriers.');
    }
